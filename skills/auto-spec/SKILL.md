...
2. Tracking issue: `gh issue create --title "[pipeline] <story_id>: <title>"` — body: story text, stage checklist (planning/interrogate → contracts → per-slice tests+build → assembly → final review), one line naming the issue as the story's control surface.
3. Create `$planning_branch` off `$feature_branch` — exactly that name; the daemon routes by it.
...
- **Legibility (driver-read test)** — can a reader find every decision that needs them within the first screen? Grasp the whole change in two minutes? Any section where history/rationale is interleaved into a decision statement? Any compression that costs clarity (fragments, unexpanded jargon, arrow chains)?
...
2. Open the planning PR (base `$feature_branch`, head `$planning_branch`, title `[planning] <story_id>: <title>`). Body: story summary, slice list, and a **"How to drive this PR"** section (line comments / review to discuss; summon token in backticks; merge = approve scope and start contract authoring).
...
4. Update `.pipeline/state.json` on `$feature_branch` (planning PR number, tracking issue, slices `"stage": "pending"`); commit, push. Comment on the tracking issue: intake complete, link the PR, next action = review it.
...
1. **What & why** — ≤4 sentences orienting a cold reader: what this change is, why now, what it deliberately is not.
